 ...
 * @return
 */
public class DesignerReserve extends BaseActivity implements OnClickListener {
    ...
    private LinearLayout name_submit_ll, balance_ll;
    ...
    public String zid, zrid, _name, type, sqId, addrSel, addr;

    private String yhName, yhId, fgId, yhqStatus, totalPay, bxStatus,
	    payMethod, balance, style_text, addr_validation;

    private HashMap<String, String> fgIdMap;

    private long timeStamp;

    private String orderNum, alipayOrderInfo;
	...
	new InfoAsync().execute(zid, new Tools().getUserId(this));
	...
	type = getIntent().getExtras().getString("type");// 预约-yy， 导航-frag
	sqId = getIntent().getExtras().getString("sqid");
	addrSel = getIntent().getExtras().getString("sel");// 1为只有商圈，需要手填地址，
							   // 2为详细地址，可以选择更改
	addr = getIntent().getExtras().getString("address");
	String time = getIntent().getExtras().getString("time");// 格式为 8.26(一)
								// 11点
	Log.e("zrid", zrid);
	Log.e("type", type);
	Log.e("_name", _name);
	...
	// } else {
	hr = time.split(" ")[2].substring(0, time.split(" ")[2].length() - 1);
	...
	if (type.equals("yy")) {
	...
	} else {// 导航, 地址需要选择， 选择之后验证地址是否在服务范围之内
	    addrTitle.setText("服务地址");
	    address.setOnClickListener(new OnClickListener() {
			...
			// TODO Auto-generated method stub
			new PopupWindowsAddr(DesignerReserve.this, parent);
		    }
		});
	}
	...
	protected String doInBackground(String... params) {
	    // TODO Auto-generated method stub
	    String url = Config.DESIGNER_RESERVE_URL + params[0] + "&uid="
		    + params[1] + "&zrid=" + zrid;
	    Log.e("url", url);
		...
		if (code.equals("1")) {
		    ...
		    hashMap.put("id", _data.getString("id"));
		    hashMap.put("pic", _data.getString("pic"));
		    hashMap.put("xm", _data.getString("xm"));
		    hashMap.put("dh", _data.getString("dh"));
		    hashMap.put("hs", _data.getString("hs"));
		    hashMap.put("jg1", _data.getString("jg1"));
		    hashMap.put("yhqid", _data.getString("yhqid"));
		    hashMap.put("yhqname", _data.getString("yhqname"));
		    yhId = _data.getString("yhqid");
		    yhName = _data.getString("yhqname");
		    balance = _data.getString("yh_zhye");
		    ...
		    fgIdMap = new HashMap<String, String>();
		    JSONArray fg = _data.getJSONArray("fg");
		    for (int i = 0, j = fg.length(); i < j; i++) {
			JSONObject job1 = fg.optJSONObject(i);
			fgList.add(job1.getString("classname"));
			fgIdMap.put(job1.getString("classname"),
				job1.getString("id"));
		    }
		    style_text = fgList.get(0);
		    fgId = fgIdMap.get(style_text);
		    ...
		    if (!type.equals("yy")
			    && addrList.get(0).get("pic").equals("1")) {
			Message msg2 = handler.obtainMessage();
			msg2.what = 1;
			msg2.obj = addrList.get(0).get("dz");
			sqId = addrList.get(0).get("sqid");
			msg2.sendToTarget();
		    }

		    JSONArray yhq = _data.getJSONArray("yhq");
		    if (yhq.length() == 0) {
			yhqStatus = "0";
		    } else {
			yhqStatus = "1";
	...
	protected void onPostExecute(String result) {
		...
		Toast.makeText(DesignerReserve.this, "获取造型师信息失败，请重试！",
	...
	protected void onPostExecute(String result) {
		...
		Toast.makeText(DesignerReserve.this, "造型师不在服务范围之内，请选择服务地址！",
			Toast.LENGTH_SHORT).show();
		addr_validation = "0";
		// finish();
	    }
	...
	protected String doInBackground(Void... params) {
	    // TODO Auto-generated method stub
	    String url = Config.DESIGNER_RESERVE_POST_URL + getOrder();
	    Log.e("url", url);
	    String data = new Tools().getURL(url);
	    System.out.println(data);
	    String code = "";
	...
	protected void onPostExecute(final String result) {
		...
		public void run() {
		    ...
		    pd.dismiss();
		    if (result.equals("1")) {
			Toast.makeText(DesignerReserve.this, "提交订单成功，请选择付款方式！",
				Toast.LENGTH_SHORT).show();
			Message msg = handler.obtainMessage();
			msg.what = 555;
			msg.sendToTarget();
		    } else {
			Toast.makeText(DesignerReserve.this, msg,
				Toast.LENGTH_SHORT).show();
		    }
		}
	    }, 3000);
	...
	protected void onPostExecute(String result) {
	    ...
	    super.onPostExecute(result);
	    pd.dismiss();
	    Toast.makeText(DesignerReserve.this, msg, Toast.LENGTH_SHORT)
		    .show();
	    if (result.equals("1")) {
		startActivity(new Intent(DesignerReserve.this,
			OrderListMain.class));
		DesignerReserve.this.finish();
	    }
	}
	...
	public void handleMessage(Message msg) {
		...
		ArrayAdapter<String> adapter = new ArrayAdapter<String>(
			DesignerReserve.this, R.layout.area_spinner_tv,
			R.id.text, fgList);
		...
		style_tv.setVisibility(View.VISIBLE);
		style_tv.setText(style_text);
		...
		pay.setOnClickListener(new OnClickListener() {
			...
			if (payMethod.equals("0")) {
			    Toast.makeText(DesignerReserve.this, "请选择支付方式！",
				    Toast.LENGTH_SHORT).show();
			} else {
	...
	public void handleMessage(Message msg) {
		...
		if (TextUtils.equals(resultStatus, "9000")) {
		    Toast.makeText(DesignerReserve.this, "支付成功",
			    Toast.LENGTH_SHORT).show();
		} else {
		    // 判断resultStatus 为非“9000”则代表可能支付失败
		    // “8000”代表支付结果因为支付渠道原因或者系统原因还在等待支付结果确认，最终交易是否成功以服务端异步通知为准（小概率状态）
		    if (TextUtils.equals(resultStatus, "8000")) {
			Toast.makeText(DesignerReserve.this, "支付结果确认中",
				Toast.LENGTH_SHORT).show();

		    } else {
			// 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
			Toast.makeText(DesignerReserve.this, "支付失败",
				Toast.LENGTH_SHORT).show();
	...
	case R.id.reserve_order_coupon_rl:
		...
		Toast.makeText(DesignerReserve.this, "没有优惠券！",
			Toast.LENGTH_SHORT).show();
	    } else {
	...
	case R.id.reserve_order_protocol:
	    startActivity(new Intent(DesignerReserve.this, InsuranceView.class));
	    break;
	case R.id.reserve_order_pay:
	    ...
	    //
	    // }
	    if(new Tools().isUserLogin(DesignerReserve.this)){
		if (validation()) {
			new PostAsync().execute();
		    }
	    }else{
		Toast.makeText(DesignerReserve.this, "请先登录！", Toast.LENGTH_SHORT).show();
		startActivity(new Intent(DesignerReserve.this, LoginActivity.class));
	    }
	...
	if (addrSel.equals("1") && type.equals("yy")) {
	    if (addr_et2.getText().toString().length() == 0) {
		Toast.makeText(DesignerReserve.this, "请填写详细地址！",
			Toast.LENGTH_SHORT).show();
		return false;
	    }
	}
	...
	try {
	    ...
	    if (type.equals("yy")) {
		if (addrSel.equals("1")) {
		    addr_utf8 = URLEncoder.encode(address.getText().toString()
			    + addr_et2.getText().toString(), "UTF-8");
		} else {
		    addr_utf8 = URLEncoder.encode(address.getText().toString(),
			    "UTF-8");
		}
	    ...
	    System.out.println(remark_utf8);
	...
	System.out.println("dt ---> " + yr + "-" + month + "-" + day + " "
		+ time);

	return yr + "-" + month + "-" + day + " " + time;
    }
	...
	public PopupWindowsAddr(Context mContext, View parent) {
	    ...
	    adapter = new AddressListAdapter(addrList, DesignerReserve.this);
